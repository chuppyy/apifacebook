#region

using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using AutoMapper;
using ITC.Domain.Commands.NewsManagers.MinusWord;
using ITC.Domain.Core.Bus;
using ITC.Domain.Core.ModelShare.StudyManagers.MinusWord;
using ITC.Domain.Interfaces;
using ITC.Domain.Interfaces.AuthorityManager.AuthorityManagerSystem;
using ITC.Domain.Interfaces.StudyManagers.MinusWord;
using NCore.Actions;
using NCore.Modals;

#endregion

namespace ITC.Application.AppService.StudyManagers.MinusWord;

/// <summary>
///     Class service loại môn học
/// </summary>
public class MinusWordAppService : IMinusWordAppService
{
#region Constructors

    /// <summary>
    ///     Hàm dựng có tham số
    /// </summary>
    /// <param name="mapper"></param>
    /// <param name="bus"></param>
    /// <param name="minusWordQueries"></param>
    /// <param name="minusWordRepository"></param>
    /// <param name="authorityManagerQueries"></param>
    /// <param name="user"></param>
    public MinusWordAppService(IMapper                  mapper,
                               IMediatorHandler         bus,
                               IMinusWordQueries        minusWordQueries,
                               IMinusWordRepository     minusWordRepository,
                               IAuthorityManagerQueries authorityManagerQueries,
                               IUser                    user)
    {
        _mapper                  = mapper;
        _bus                     = bus;
        _minusWordQueries        = minusWordQueries;
        _minusWordRepository     = minusWordRepository;
        _authorityManagerQueries = authorityManagerQueries;
        _user                    = user;
    }

#endregion

    /// <inheritdoc cref="Add" />
    public async Task<bool> Add(MinusWordEventModel model)
    {
        var addCommand = _mapper.Map<AddMinusWordCommand>(model);
        await _bus.SendCommand(addCommand);
        model.Id            = addCommand.Id;
        model.ResultCommand = addCommand.ResultCommand;
        return model.ResultCommand;
    }

    /// <inheritdoc cref="Delete" />
    public async Task<bool> Delete(DeleteModal model)
    {
        var deleteCommand = new DeleteMinusWordCommand(model.Models);
        await _bus.SendCommand(deleteCommand);
        model.ResultCommand = deleteCommand.ResultCommand;
        return model.ResultCommand;
    }

    /// <inheritdoc cref="GetById" />
    public MinusWordEventModel GetById(Guid id)
    {
        var iReturn = _minusWordRepository.GetAsync(id).Result;
        if (iReturn == null) return null;

        return new MinusWordEventModel
        {
            Id          = iReturn.Id,
            Root        = iReturn.Root,
            Replace     = iReturn.Replace,
            Description = iReturn.Description
        };
    }

    /// <inheritdoc cref="Update" />
    public async Task<bool> Update(MinusWordEventModel model)
    {
        var addCommand = _mapper.Map<UpdateMinusWordCommand>(model);
        await _bus.SendCommand(addCommand);
        model.ResultCommand = addCommand.ResultCommand;
        return model.ResultCommand;
    }

    /// <inheritdoc cref="GetCombobox" />
    public Task<IEnumerable<ComboboxModal>> GetCombobox(string vSearch, int groupTable)
    {
        return _minusWordQueries.GetCombobox(vSearch, groupTable, _user.ProjectId);
    }

    /// <inheritdoc cref="GetPaging" />
    public async Task<IEnumerable<MinusWordPagingDto>> GetPaging(PagingModel model)
    {
        return await Task.Run(() =>
        {
            // Lấy dữ liệu quyền của người dùng
            var iPermission =
                _authorityManagerQueries
                    .GetPermissionByMenuManagerValue(model.ModuleIdentity, _user.UserId)
                    .Result;
            // Xử lý dữ liệu
            model.ProjectId = _user.ProjectId;
            var lData = (List<MinusWordPagingDto>)_minusWordQueries.GetPaging(model).Result;
            // Trả về Actions
            foreach (var item in lData)
                item.Actions =
                    ActionPagingWorkManager.ActionAuthoritiesNews(iPermission,
                                                                  item.OwnerId,
                                                                  _user.UserId,
                                                                  false,
                                                                  item.StatusId);
            return lData;
        });
    }

#region Fields

    private readonly IMediatorHandler         _bus;
    private readonly IMinusWordQueries        _minusWordQueries;
    private readonly IMinusWordRepository     _minusWordRepository;
    private readonly IAuthorityManagerQueries _authorityManagerQueries;
    private readonly IUser                    _user;
    private readonly IMapper                  _mapper;

#endregion
}