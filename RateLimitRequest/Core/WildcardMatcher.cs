...
#region Methods
    ...
    public static bool IsMatch(this string value, string pattern, char singleWildcard = '?',
                               char        multipleWildcard = '*')
    {
        var inputPosStack =
            new int[(value.Length + 1) *
                    (pattern.Length +
                     1)]; // Stack containing input positions that should be tested for further matching
        var patternPosStack =
        ...
        var pointTested =
            new bool[value.Length + 1,
                pattern.Length +
                1]; // Each true value indicates that input position vs. pattern position has been tested
        ...
        while (stackPos >= 0 && !matched)
        {
            ...
            patternPos =
                patternPosStack
                    [stackPos--]; // Matching will succeed if rest of the input string matches rest of the pattern
